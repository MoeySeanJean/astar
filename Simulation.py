...
DEFAULT_RENDER_STEPS_LIMIT = 200
SAVE_VIDEO_DIR = "./data/vids"
SAVE_ACTIONS_DIR = "./data/actions"
INSTRUCTION = os.environ.get("INSTRUCTION", "touch the red bowl on the table")
...
def openvla(
    ...
    if skip_pygame:
        ...
        if "arm_action" in env.action_space.spaces:
            ...
            raise ValueError("Arm action not found in action space")

        EE_FACTOR = 10000
        arm_action = np.zeros(3)
        arm_action[0] = action[0] * EE_FACTOR
        arm_action[1] = action[1] * EE_FACTOR
        arm_action[2] = action[2] * EE_FACTOR
        magic_grasp = int(round(action[6]))

        args = {
            arm_key: arm_action,
            grip_key: magic_grasp,
        }

        return step_env(env, "arm_action", args), [*arm_action, magic_grasp], False
...
def zoom_and_pad(frame, target_h, target_w):
    h, w, _ = frame.shape

    # Compute scale factor to fit entire image in target resolution
...
def play_env(env, args, config):
    ...
    env._sim.articulated_agent.base_pos = np.array([0.5, 0.121, 0.74])
    env._sim.articulated_agent.base_rot = np.pi/2
    ...
    while True:
        ...
        step_result, arm_action, end_ep = openvla(
            args.no_render,
            args.cfg,
        ...
        if args.save:
            if "head_rgb" in obs:
                all_obs.append(zoom_and_pad(obs["head_rgb"], 720, 1280))
            else:
                logger.warning("No 'head_rgb' sensor in observation; skipping save for this frame.")
